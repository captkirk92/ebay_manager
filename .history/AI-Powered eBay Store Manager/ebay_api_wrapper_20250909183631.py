...
def get_listing_details(item_id):
    ...
    try:
        ...
        try:
            # The response is already a parsed dictionary
            if 'Item' in response:
                item = response['Item']
                ...
                item_data = {
                    ...
                    'CurrentPrice': float(item.get('StartPrice', {}).get('Value', 0)) if item.get('StartPrice') else 0.0,
                    'ViewItemURL': item.get('ViewItemURL', ''),
                    'GalleryURL': item.get('GalleryURL', ''),
                    'PictureURL': []
                }
                
                # Extract picture URLs from PictureDetails
                picture_details = item.get('PictureDetails', {})
                if picture_details:
                    # Handle both single URL and list of URLs
                    picture_urls = picture_details.get('PictureURL', [])
                    if isinstance(picture_urls, str):
                        item_data['PictureURL'] = [picture_urls]
                    elif isinstance(picture_urls, list):
                        item_data['PictureURL'] = picture_urls
                    
                    # Also get GalleryURL from PictureDetails if available
                    if picture_details.get('GalleryURL'):
                        item_data['GalleryURL'] = picture_details['GalleryURL']
                
                # If no pictures in PictureDetails, check top level
                if not item_data['PictureURL'] and item.get('PictureURL'):
                    picture_url = item['PictureURL']
                    if isinstance(picture_url, str):
                        item_data['PictureURL'] = [picture_url]
                    elif isinstance(picture_url, list):
                        item_data['PictureURL'] = picture_url
