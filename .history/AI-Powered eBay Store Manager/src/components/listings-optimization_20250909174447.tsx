...
import { Search, AlertTriangle, TrendingUp, Edit, DollarSign, Eye, Loader2 } from "lucide-react";
...
export function ListingsOptimization() {
  const { listings, isListingsLoading, listingsError, analytics, isAnalyticsLoading } = useEbayData();
  const [listingOptimizations, setListingOptimizations] = useState<ListingOptimization[]>([]);
  const [optimizationStats, setOptimizationStats] = useState<OptimizationStat[]>([]);
  ...
  const analyzeTitleOptimization = (title: string) => {
    ...
    const commonBrands = ['Apple', 'Samsung', 'Nike', 'Sony', 'Canon', 'Dell', 'HP', 'Lenovo'];
    const hasBrand = commonBrands.some(brand => title.toLowerCase().includes(brand.toLowerCase()));
    if (!hasBrand) {
      score -= 15;
    ...
    const conditionWords = ['new', 'used', 'refurbished', 'excellent', 'mint', 'like new'];
    const hasCondition = conditionWords.some(word => title.toLowerCase().includes(word));
    if (!hasCondition) {
      score -= 10;
  ...
  useEffect(() => {
    if (listings?.listings && !isListingsLoading) {
      ...
      const optimizations: ListingOptimization[] = listings.listings.map((listing) => {
      ...
      const needsWork = optimizations.filter(opt => opt.severity === 'high' || opt.severity === 'medium').length;
      const avgScore = optimizations.length > 0 
        ? Math.round(optimizations.reduce((sum, opt) => sum + opt.currentScore, 0) / optimizations.length)
        : 0;
      const potentialIncrease = optimizations.reduce((sum, opt) => {
        const current = parseFloat(opt.currentPrice.replace(/[^0-9.]/g, ''));
        const suggested = parseFloat(opt.suggestedPrice.replace(/[^0-9.]/g, ''));
        return sum + (suggested - current);
      }, 0);
      const lowVisibility = optimizations.filter(opt => opt.views < 100).length;
        ...
        <div className="flex items-center justify-center py-12">
          <div className="text-center space-y-4">
            ...
            <div className="text-lg font-medium">
              {isListingsLoading ? "Loading listings..." : "Analyzing listings for optimization opportunities..."}
            </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-center space-y-4">
              ...
              <div className="text-lg font-medium">No active listings found</div>
              <div className="text-sm text-muted-foreground">
      ...
      <Card>
        ...
        <CardContent>
          <Tabs defaultValue="all">
            <TabsList>
              <TabsTrigger value="all">All Issues ({listingOptimizations.length})</TabsTrigger>
              <TabsTrigger value="high">
                High Priority ({listingOptimizations.filter(l => l.severity === 'high').length})
              </TabsTrigger>
              <TabsTrigger value="pricing">
                Pricing ({listingOptimizations.filter(l => l.suggestions.some(s => s.includes('price'))).length})
              </TabsTrigger>
              <TabsTrigger value="seo">
                SEO Issues ({listingOptimizations.filter(l => l.currentScore < 75).length})
              </TabsTrigger>
            </TabsList>
                ...
                <TableBody>
                  {listingOptimizations.map((listing) => (
                    <TableRow key={listing.id}>
                      <TableCell>
                        <div>
                          <div className="font-medium text-sm max-w-xs truncate">{listing.title}</div>
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          ...
                          <Button 
                            ...
                            variant="outline" 
                            className="text-xs"
                            onClick={() => window.open(`https://www.ebay.com/itm/${listing.id}`, '_blank')}
                          >
                            View
                          </Button>
                        </div>
            ...
            <TabsContent value="pricing">
              <div className="text-center py-8 text-muted-foreground">
                Pricing optimization suggestions based on current market data will appear here.
              </div>
            </TabsContent>
            
            <TabsContent value="seo">
              <div className="text-center py-8 text-muted-foreground">
                SEO improvement recommendations based on listing analysis will appear here.
              </div>
            </TabsContent>
          </Tabs>
        </CardContent>
          ...
          <CardContent className="space-y-4">
            <div className="p-4 border rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="font-mono bg-muted p-2 rounded">{listingOptimizations[0]?.title || "Sample listing title"}</div>
                ...
                <div className="flex flex-wrap gap-1 mt-2">
                  {(listingOptimizations[0]?.suggestions || []).map((suggestion, i) => (
            ...
            <div className="p-4 border rounded-lg">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="font-bold">{listingOptimizations[0]?.currentPrice || "$19.99"}</span>
                </div>
                <div className="flex justify-between">
                  <span>Suggested Price:</span>
                  <span className="font-bold text-green-600">{listingOptimizations[0]?.suggestedPrice || "$24.99"}</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <Badge variant="secondary">{listingOptimizations[0]?.competition || "47"} listings</Badge>
                </div>
              </div>
            </div>
          </CardContent>
