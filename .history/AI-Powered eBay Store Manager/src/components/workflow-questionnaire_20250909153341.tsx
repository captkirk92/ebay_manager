...
export function WorkflowQuestionnaire({
  onComplete,
  onCancel,
}: WorkflowQuestionnaireProps) {
  ...
  const steps = [
    ...
    { title: "Special Handling", description: "Any special requirements?" },
    {
      ...
      description: "Workflow tools and completion",
    },
  ];
    ...
    switch (currentStep) {
        ...
        return (
          <div className="space-y-4">
            ...
            <RadioGroup
              ...
              onValueChange={(value) =>
                handleAnswerChange("productType", value)
              }
            >
      ...
      case 1:
        return (
          <div className="space-y-4">
            <Label className="text-lg font-semibold">
              Do your products require customization or buyer input before
              shipping?
            </Label>
            <RadioGroup
              value={answers.customization.toString()}
              onValueChange={(value) =>
                handleAnswerChange("customization", value === "true")
              }
            >
              <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-muted/50">
                <RadioGroupItem value="true" id="customization-yes" />
                <Label htmlFor="customization-yes" className="cursor-pointer">
                  Yes - I need buyer input (artwork, personalization, etc.)
                </Label>
              </div>
              <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-muted/50">
                <RadioGroupItem value="false" id="customization-no" />
                <Label htmlFor="customization-no" className="cursor-pointer">
                  No - Items are ready to ship as-is
                </Label>
              </div>
            </RadioGroup>
            {answers.customization && (
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    handleAnswerChange("customizationDetails", e.target.value)
                  }
                />
              </div>
            )}
      ...
      case 2:
        return (
          <div className="space-y-4">
            <Label className="text-lg font-semibold">
              How do you usually package your items for shipping?
            </Label>
            <div className="space-y-2">
                  ...
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        ...
                        handleAnswerChange(
                          "packaging",
                          answers.packaging.filter((p) => p !== option.value)
                        );
                      }
                    }}
                  />
            ...
            <div className="p-4 bg-blue-50 rounded-lg">
              ...
              <div className="flex items-center space-x-2 mt-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleAnswerChange("standardEnvelope", checked)
                  }
                />
                <Label
                  htmlFor="standard-envelope"
                  className="text-sm cursor-pointer"
                >
                  I use eBay Standard Envelope for small/light items (≤3 oz,
                  specific dimensions)
                </Label>
              </div>
      ...
      case 3:
        return (
          <div className="space-y-4">
            <Label className="text-lg font-semibold">
              What shipping carriers and methods do you usually use?
            </Label>
            <div className="space-y-2">
              {SHIPPING_METHODS.map((method) => (
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        handleAnswerChange("shippingMethods", [
                          ...answers.shippingMethods,
                          method.value,
                        ]);
                      } else {
                        handleAnswerChange(
                          "shippingMethods",
                          answers.shippingMethods.filter(
                            (s) => s !== method.value
                          )
                        );
                      }
      ...
      case 4:
        return (
          <div className="space-y-4">
            ...
            <RadioGroup
              value={answers.fulfillmentType}
              onValueChange={(value) =>
                handleAnswerChange("fulfillmentType", value)
              }
            >
              <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-muted/50">
                <RadioGroupItem value="self_fulfill" id="self-fulfill" />
                <Label htmlFor="self-fulfill" className="cursor-pointer">
                  I pack and ship items myself
                </Label>
              </div>
              <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-muted/50">
                <RadioGroupItem
                ...
                />
                <Label htmlFor="fulfillment-service" className="cursor-pointer">
                  I use a fulfillment service (3PL)
                </Label>
              </div>
              <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-muted/50">
                <RadioGroupItem value="dropship" id="dropship" />
                <Label htmlFor="dropship" className="cursor-pointer">
                  I dropship from suppliers
                </Label>
              </div>
            </RadioGroup>
          </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-2 p-3 border rounded-lg hover:bg-muted/50">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleAnswerChange("thankYouMessage", checked)
                  }
                />
                <Label htmlFor="thank-you" className="cursor-pointer">
                  I send a thank you or confirmation message when an order is
                  received
                </Label>
              </div>
            </div>
      ...
      case 6:
        return (
          <div className="space-y-4">
            <Label className="text-lg font-semibold">
              Are there any special handling requirements for your products?
            </Label>
            <div className="space-y-2">
              {SPECIAL_HANDLING.map((handling) => (
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        handleAnswerChange("specialHandling", [
                          ...answers.specialHandling,
                          handling.value,
                        ]);
                      } else {
                        handleAnswerChange(
                          "specialHandling",
                          answers.specialHandling.filter(
                            (h) => h !== handling.value
                          )
                        );
                      }
                  ...
                  />
                  <Label htmlFor={handling.value} className="cursor-pointer">
                    {handling.label}
                  </Label>
                </div>
              ))}
      ...
      case 7:
        return (
          <div className="space-y-4">
            <Label className="text-lg font-semibold">
              What tools or software do you use in your workflow?
            </Label>
            <div className="space-y-2">
              {TOOLS.map((tool) => (
                <div
                  ...
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      if (checked) {
                        ...
                        handleAnswerChange(
                          "tools",
                          answers.tools.filter((t) => t !== tool.value)
                        );
                      }
                    }}
                  />
                  <Label htmlFor={tool.value} className="cursor-pointer">
                    {tool.label}
                  </Label>
                </div>
              ))}
            ...
            <div className="space-y-2">
              ...
              <Select
                value={answers.verificationMethod}
                onValueChange={(value) =>
                  handleAnswerChange("verificationMethod", value)
                }
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  <SelectItem value="tracking_uploaded">
                    Once tracking is uploaded
                  </SelectItem>
                  <SelectItem value="buyer_confirms">
                    Once buyer confirms delivery
                  </SelectItem>
                  <SelectItem value="manual_check">
                    Once I manually check it off
                  </SelectItem>
                  <SelectItem value="fulfillment_service">
                    Fulfillment service handles it
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div className="space-y-2">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleAnswerChange("customRequirements", e.target.value)
                }
              />
            </div>
          </div>
...
function createWorkflowFromAnswers(answers: QuestionnaireAnswers) {
  ...
  if (answers.thankYouMessage) {
    steps.push({
      ...
      templateMessage:
        "Hi [Buyer], thank you for your order! We've received your order and will begin processing it shortly. Order #: [OrderID]",
    });
  }
  ...
  if (answers.customization) {
    ...
    steps.push({
      ...
      action: "getMessages",
      instructions:
        "Check buyer messages for required customization details and files.",
    });
  }
  ...
  if (answers.customization || answers.tools.length > 0) {
    steps.push({
      ...
      action: "manualStep",
      instructions: `Complete production using: ${answers.tools.join(", ")}. ${
        answers.customization ? "Process customization details from buyer." : ""
      }`,
    });
  }
  ...
  steps.push({
    ...
    instructions: `Generate shipping label using ${answers.shippingMethods.join(
      " or "
    )} and upload tracking.`,
  });
...
function generatePackagingInstructions(answers: QuestionnaireAnswers): string {
  ...
  if (answers.standardEnvelope) {
    instructions +=
      '• For eBay Standard Envelope: Use flexible paper envelope (3.5" × 5" to 6.125" × 11.5", thickness <0.25"), NO bubble mailers allowed\n';
  }

  // Packaging type specific instructions
  if (answers.packaging.includes("corrugated_box")) {
    instructions +=
      "• For corrugated boxes: Use new box slightly larger than item, wrap with bubble wrap/foam, no empty space to prevent shifting\n";
  }

  if (answers.packaging.includes("rigid_mailer")) {
    instructions +=
      "• For rigid mailers: Ensure item is flat and won't bend, add protection if needed\n";
  }

  if (answers.packaging.includes("double_box")) {
    instructions +=
      '• For double boxing: Inner box with cushioning, outer box with 2" padding on all sides\n';
  }
  ...
  if (answers.specialHandling.includes("high_value")) {
    instructions +=
      "• Add insurance and signature confirmation if item > $200\n";
  }

  // General eBay rules
  instructions +=
    '• Use 2" wide reinforced tape, remove old labels if reusing boxes\n';
  instructions +=
    "• Accurately measure and round up dimensions/weight to avoid surcharges\n";
