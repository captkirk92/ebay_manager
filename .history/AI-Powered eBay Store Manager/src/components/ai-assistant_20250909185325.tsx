...
export function AIAssistant({
  ...
  const [showOrderHealth, setShowOrderHealth] = useState(false);
  ...
  const quickActions = [
    ...
    { text: "Order analysis", icon: DollarSign, color: "text-green-400" },
    { text: "Workflow setup", icon: Workflow, color: "text-orange-400" },
    { text: "Analyze inventory", icon: Package, color: "text-blue-400" },
    { text: "Optimize listings", icon: Target, color: "text-purple-400" },
  ];
  ...
  const getAIResponse = (userInput: string): string => {
    ...
    if (
      ...
      input.includes("fulfillment") ||
      input.includes("order process")
    ) {
      return `🔄 **Order Fulfillment Workflows**: I can help you set up automated workflows for processing orders efficiently!\n\n**Available Options:**\n• **Pre-made Templates**: Choose from 8+ templates (stickers, clothing, electronics, etc.)\n• **Custom Workflow**: Build a personalized process for your specific business\n• **eBay-Compliant**: All workflows include proper packaging rules and API integrations\n\n**Features:**\n• Step-by-step guidance for each order\n• Automated messaging to buyers\n• eBay API integration for shipping\n• Progress tracking and completion verification\n\nClick "Workflow setup" below to get started, or ask me about specific workflow needs!`;
    }
  ...
  const handleQuickAction = (action: string) => {
    ...
    } else if (action === "Order analysis") {
      setShowOrderHealth(true);
    } else {
      ...
      <div className="fixed inset-0 z-50 bg-background">
        <div className="h-full flex flex-col">
          <div className="flex items-center justify-between p-4 border-b">
            <h1 className="text-2xl font-bold">Order Financial & Fulfillment Analysis</h1>
            <Button variant="outline" onClick={() => setShowOrderHealth(false)}>
              Back to AI Assistant
            </Button>
          </div>
          <div className="flex-1 overflow-auto p-6">
            <OrderHealthDashboard />
          </div>
        </div>
      </div>
    );
