...
export function AIAssistant({
  ...
  const { storeSummary, orders, listings, analytics, health, isLoading, error } =
    useEbayData();
  ...
  const generateIntelligentInsights = () => {
    ...
    if (health && health.analysis) {
      ...
      const healthEmoji = healthScore >= 90 ? "🟢" : healthScore >= 80 ? "🟡" : healthScore >= 70 ? "🟠" : "🔴";
      insights.push({
        id: "health-score",
        type: "ai",
        content: `${healthEmoji} **Store Health Score: ${healthScore}/100** (${healthStatus})\n\n${health.analysis.strengths.length > 0 ? `**Strengths:**\n${health.analysis.strengths.map(s => `• ${s}`).join('\n')}\n\n` : ''}${health.analysis.concerns.length > 0 ? `**Areas for Improvement:**\n${health.analysis.concerns.map(c => `• ${c}`).join('\n')}\n\n` : ''}**Today's Action Items:**\n${health.analysis.action_items.slice(0, 3).map((item, i) => `${i + 1}. ${item}`).join('\n')}`,
  ...
  const getAIResponse = (userInput: string): string => {
    ...
    if (input.includes("health") || input.includes("performance") || input.includes("score")) {
      if (health && health.analysis) {
        const healthScore = health.health_score;
        const healthStatus = health.analysis.overall_health;
        const healthEmoji = healthScore >= 90 ? "🟢" : healthScore >= 80 ? "🟡" : healthScore >= 70 ? "🟠" : "🔴";
        
        return `${healthEmoji} **Store Health Analysis** (Score: ${healthScore}/100)\n\n**Overall Status:** ${healthStatus}\n\n**Key Strengths:**\n${health.analysis.strengths.map(s => `• ${s}`).join('\n')}\n\n**Areas Needing Attention:**\n${health.analysis.concerns.map(c => `• ${c}`).join('\n')}\n\n**Priority Actions for Today:**\n${health.analysis.action_items.slice(0, 3).map((item, i) => `${i + 1}. ${item}`).join('\n')}\n\n${healthScore < 80 ? '⚠️ Your store health needs immediate attention. Focus on the action items above to improve your score.' : '✅ Your store is performing well! Keep monitoring these metrics to maintain your high score.'}`;
      } else {
