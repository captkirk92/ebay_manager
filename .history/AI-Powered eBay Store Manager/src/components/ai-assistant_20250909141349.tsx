...
export function AIAssistant({ isExpanded = false, onToggle }: AIAssistantProps) {
  const { storeSummary, orders, listings, analytics, isLoading, error } = useEbayData();
  ...
  useEffect(() => {
    if (!isLoading && !hasAnalyzedData && (storeSummary || listings || analytics)) {
      generateIntelligentInsights();
      setHasAnalyzedData(true);
    }
  }, [isLoading, storeSummary, listings, analytics, hasAnalyzedData]);
  ...
  const generateIntelligentInsights = () => {
    ...
    insights.push({
      id: 'welcome',
      type: 'ai',
      content: `👋 Hello! I'm your AI Co-Manager for **${storeSummary?.store_name || 'midnight sun printing'}**. I've analyzed your live store data and found several opportunities to improve your performance.`,
    ...
    if (listings && Array.isArray(listings)) {
      ...
      const activeListings = listings.filter(l => parseInt(l.quantity) > 0).length;
      ...
      if (totalListings > 0) {
        if (activeListings < totalListings) {
          insights.push({
            id: 'inventory-alert',
            type: 'ai',
            content: `⚠️ **Inventory Alert**: ${totalListings - activeListings} of your ${totalListings} listings are out of stock. Restocking these could increase your visibility and sales potential.`,
    ...
    if (analytics) {
      ...
      if (totalRevenue > 0) {
        insights.push({
          ...
          type: 'ai',
          content: `📊 **Performance Analysis**: You've generated $${totalRevenue.toFixed(2)} from ${totalOrders} orders (AOV: $${avgOrderValue.toFixed(2)}). ${avgOrderValue > 25 ? 'Your AOV is strong!' : 'Consider bundling products to increase average order value.'}`,
        ...
        if (avgOrderValue < 30) {
          insights.push({
            id: 'aov-suggestion',
            type: 'ai',
            content: `💰 **Revenue Opportunity**: Your average order value is $${avgOrderValue.toFixed(2)}. By creating product bundles or offering quantity discounts, you could potentially increase this by 20-40%, adding $${(totalRevenue * 0.3).toFixed(2)}+ in additional revenue.`,
  ...
  const quickActions = [
    ...
    { text: "Revenue insights", icon: BarChart3, color: "text-purple-400" },
    { text: "Growth strategies", icon: Zap, color: "text-yellow-400" }
  ];
  ...
  const getAIResponse = (userInput: string): string => {
    ...
    if (input.includes('inventory') || input.includes('analyze inventory')) {
      const outOfStock = listings?.filter(l => parseInt(l.quantity) === 0).length || 0;
      const lowStock = listings?.filter(l => {
        const qty = parseInt(l.quantity);
        return qty > 0 && qty <= 5;
      }).length || 0;
      
      return `📦 **Live Inventory Analysis**: Out of ${totalListings} listings:\n• ${outOfStock} items are out of stock\n• ${lowStock} items have low stock (≤5 units)\n• ${totalListings - outOfStock - lowStock} items have healthy stock levels\n\n${outOfStock > 0 ? `⚠️ Priority: Restock ${outOfStock} out-of-stock items to maintain visibility.` : '✅ No critical stock issues detected.'}`;
    }
    
    if (input.includes('optimize') || input.includes('listings') || input.includes('seo')) {
      const withVariations = listings?.filter(l => l.hasVariations).length || 0;
      const customStickers = listings?.filter(l => l.title.toLowerCase().includes('custom')).length || 0;
    ...
    if (input.includes('revenue') || input.includes('insights') || input.includes('growth')) {
      if (totalRevenue > 0) {
        const projectedGrowth = totalRevenue * 0.4;
        return `💰 **Live Revenue Analysis** ($${totalRevenue.toFixed(2)} current):\n\n📊 **Current Performance**:\n• ${totalOrders} orders completed\n• $${avgOrderValue.toFixed(2)} average order value\n• $${(totalRevenue/totalListings).toFixed(2)} revenue per listing\n\n🚀 **Growth Opportunities**:\n1. Bundle products → +$${(projectedGrowth * 0.3).toFixed(2)} potential\n2. Upsell variations → +$${(projectedGrowth * 0.4).toFixed(2)} potential\n3. Seasonal promotions → +$${(projectedGrowth * 0.3).toFixed(2)} potential\n\n**Total Growth Potential: +$${projectedGrowth.toFixed(2)}**`;
      } else {
