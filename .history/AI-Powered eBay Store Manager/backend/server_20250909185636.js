...
// Get detailed listing information with images
app.get("/api/store/listing/:itemId", async (req, res) => {
  try {
...
app.get("/api/orders/analyze/:orderId", async (req, res) => {
  try {
    ...
    pythonProcess.on("close", (code) => {
      if (code === 0) {
        try {
          ...
          res.status(500).json({
            error: "Failed to parse order analysis",
            details: parseError.message,
          });
        }
      } else {
...
app.get("/api/orders/health-report", async (req, res) => {
  try {
    const pythonProcess = spawn("python3", [
      "../ebay_api_wrapper.py",
      "--order-health",
    ...
    pythonProcess.on("close", (code) => {
      if (code === 0) {
        try {
          ...
          res.status(500).json({
            error: "Failed to parse order health report",
            details: parseError.message,
          });
        }
      } else {
        res.status(500).json({
          error: "Failed to generate order health report",
          code: code,
          stderr: errorData,
        });
      }
