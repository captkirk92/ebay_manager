...
export function WorkflowExecutor({
  ...
  const handleStepError = (stepIndex: number, error: string) => {
    setStepStatuses((prev) =>
      prev.map((status, index) =>
        index === stepIndex ? { ...status, inProgress: false, error } : status
      )
    );
  };
  ...
  const handlePauseResume = () => {
    ...
    if (!isPaused) {
      setStepStatuses((prev) =>
        prev.map((status) => ({ ...status, inProgress: false }))
      );
    }
  };
        ...
        <CardHeader>
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span>Progress</span>
              <span>
                {completedSteps} of {workflow.length} steps completed
              </span>
            </div>
        ...
        <Card className="border-green-200 bg-green-50">
          <CardContent className="p-6 text-center">
            ...
            <p className="text-green-700">
              All steps have been completed successfully. Order {orderId} is
              ready for delivery.
            </p>
          </CardContent>
        </Card>
